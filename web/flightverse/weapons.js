...
import {
  ...
  segmentSphereHit,
} from '/flightverse/collision-math.js?v=298';
import {
  EffectPool,
  impactTransform,
  isContinuousWeapon,
  projectileDirection,
} from '/flightverse/aiming.js?v=298';
...
export function createWeapons(scene, {
  ...
  const emit = (tex, pos, vel, size0, size1, life, blending, o = {}) => {
    ...
    S.parts.push(poolEffect(o.effect || (o.smoke ? 'smoke' : 'spark'), part, () => group.remove(sp)));
    return part;
  };
  ...
  function fireAftermath(p) {                 // fuego residual que arde y muere
    ...
    group.add(light);
    S.fires.push(poolEffect('fire', { p: p.clone(), t: 0, life: 5.5, acc: 0, light }, () => group.remove(light)));
  }
  ...
  function explode(hitOrPoint, big = 1) {
    ...
    group.add(light);
    S.parts.push(poolEffect('fire', { light, t: 0, life: 0.22 }, () => group.remove(light)));
    ...
    for (let i = 0; i < 12; i++) {
      ...
      S.frags.push(poolEffect('fragment', fragment, () => group.remove(m2)));
    }
    // brasas: Points con textura suave (adiós cuadrados) + gravedad
    {
      ...
      group.add(pts);
      S.parts.push(poolEffect('spark', { pts, vel, t: 0, life: 0.9, grav: 34 }, () => group.remove(pts)));
    }
    // streaks: chispas estiradas por velocidad (LineSegments, técnica quarks)
    {
      ...
      group.add(ln);
      S.parts.push(poolEffect('spark', { ln, vel, t: 0, life: 0.8, grav: 30 }, () => group.remove(ln)));
    }
    ...
    group.add(ring);
    S.parts.push(poolEffect('fire', { ring, t: 0, life: 0.5, big }, () => group.remove(ring)));
    ...
    group.add(ring2);
    S.parts.push(poolEffect('fire', { ring: ring2, t: 0, life: 0.62, big: big * 0.92 }, () => group.remove(ring2)));
    // scorch persistente
    ...
    group.add(sc);
    const decal = poolEffect('decal', { m: sc }, () => group.remove(sc));
    S.decals.push(decal);
  ...
  function smash(node, color, blastP, impactNormal = null) {
    ...
    if (kit) {
      ...
      for (const f of frags) {
        ...
        S.frags.push(poolEffect('fragment', fragment, () => group.remove(f)));
      }
    ...
    for (let i = 0; i < 18; i++) {
      ...
      S.frags.push(poolEffect('fragment', fragment, () => group.remove(m)));
    }
  }
  ...
  return {
    ...
    fire(pos, aim = null, pitch = -0.05) {
      ...
      if (isContinuousWeapon(S.weapon)) {
        ...
        S.bullets.push(poolEffect('tracer', bullet, () => group.remove(b)));
        audio?.mg?.();
        return true;
      }
    ...
    update(dt, hittables) {
      ...
      for (let i = S.bullets.length - 1; i >= 0; i--) {
        ...
        if (dead) { group.remove(B.m); releaseEffect(B); S.bullets.splice(i, 1); }
      }
      // ── misiles ──
      ...
      for (let i = S.frags.length - 1; i >= 0; i--) {
        ...
        if ((grounded && F.vel.length() < 0.9) || F.t > 8) {   // se asienta → escombro estático
          ...
          const rubble = poolEffect('rubble', { m: F.m }, () => group.remove(F.m));
