...
async function preloadEnemyGlb(type, v) {
  try {
    ...
    if (!GLTFLoader) ({ GLTFLoader } = await import('/vendor/three-addons180/loaders/GLTFLoader.js?v=211'));
    if (!SkelUtils) SkelUtils = await import('/vendor/three-addons180/utils/SkeletonUtils.js?v=211');
    const g = await new GLTFLoader().loadAsync(`/assets/enemies/${type}.glb?v=211`);
