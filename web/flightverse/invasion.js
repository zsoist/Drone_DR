...
import {
  ...
  selectEnemyLod,
  steerGroundEnemy,
} from '/flightverse/invasion-policy.js?v=333';
...
export function createInvasion(scene, {
  ...
  async function loadCatalog() {
    if (!catalogPromise) {
      catalogPromise = fetch('/assets/enemies/enemy_catalog.json?v=333', { cache: 'no-store' })
        .then(response => {
  ...
  async function loadModel(type, lod) {
    ...
    const promise = (async () => {
      try {
        ...
        if (!GLTFLoader) ({ GLTFLoader } = await import('/vendor/three-addons180/loaders/GLTFLoader.js?v=333'));
        if (!SkelUtils) SkelUtils = await import('/vendor/three-addons180/utils/SkeletonUtils.js?v=333');
        const gltf = await new GLTFLoader().loadAsync(`/assets/enemies/${file}?v=333`);
