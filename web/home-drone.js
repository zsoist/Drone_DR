...
import { GLTFLoader } from '/vendor/three-addons180/loaders/GLTFLoader.js?v=333';
import { HOME_DRONE_VIEW, homeDroneFrame } from './home-drone-motion.js?v=333';
...
export async function mountHomeDrone(selector = '#home-drone-stage') {
  ...
  const rotors = [];
  try {
    const gltf = await new GLTFLoader().loadAsync('/assets/drone.glb?v=333');
    model = gltf.scene;
